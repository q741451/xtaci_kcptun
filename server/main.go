...
func main() {
	...
	myApp.Action = func(c *cli.Context) error {
		...
		// udp stack, always on
		udpLis, err := kcp.ListenWithOptions(config.Listen, block, config.DataShard, config.ParityShard)
		checkError(err)
		wg.Add(1)
		go loop(udpLis)
		...
		if config.TCP {
			...
			if err != nil {
				...
				tcpLis, err := kcp.ServeConn(block, config.DataShard, config.ParityShard, conn)
				checkError(err)
				wg.Add(1)
				go loop(tcpLis)
			}
