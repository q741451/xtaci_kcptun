...
This is the real v5.2.8 release source (`crypt.go`, `entropy.go`, `fec.go`,
`kcp.go`, `readloop_generic.go`, `sess.go`, `snmp.go`, `updater.go` are
byte-identical to upstream; test files and README images were dropped since
they're not needed to build). `go.mod` was added because Go modules requires
a `replace` target to be a real module.

## The one change: `readloop_linux.go`
...
Nothing else changed. In particular this deliberately does NOT pull in the
